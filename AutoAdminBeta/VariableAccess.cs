...
namespace eLF_AutoAdmin
{
		...
		public static Dictionary<string, Action<string[]>> getVariable = new Dictionary<string, Action<string[]>>()
	{
		...
		//new adjustments
		{"CannonClaimedMessage",get_CannonClaimedMessage },
		{"ArtilleryPrioritySystemEnabled",get_ArtilleryPrioritySystemEnabled }
	};

		private static void get_ArtilleryPrioritySystemEnabled(string[] obj)
		{
			string val = AutoAdmin.ArtyPrioritySystemEnabled.ToString();
			ConsoleController.broadcast_prefix(string.Format(dMessage, obj[1], val), AutoAdmin.f1MenuInputField);
		}

		private static void get_CannonClaimedMessage(string[] obj)
		{
			string val = MessagePresets.CannonClaimedMessage.ToString();
			ConsoleController.broadcast_prefix(string.Format(dMessage, obj[1], val), AutoAdmin.f1MenuInputField);
		}
