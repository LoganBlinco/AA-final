...
namespace eLF_AutoAdmin
{
        ...
        public static Dictionary<string, Action<string>> commandDictionary = new Dictionary<string, Action<string>>()
    {
        ...
        {"CannonClaimedMessage",variable_CannonClaimedMessage },
        {"ArtilleryPrioritySystemEnabled",variable_ArtilleryPrioritySystemEnabled }
    };

        private static void variable_CannonClaimedMessage(string obj)
        {
            MessagePresets.CannonClaimedMessage = obj;
            Debug.Log("Changed variable: CannonClaimedMessage to value: " + obj);
        }

        private static void variable_ArtilleryPrioritySystemEnabled(string obj)
        {
            bool state;
            if (bool.TryParse(obj, out state))
            {
                AutoAdmin.ArtyPrioritySystemEnabled = state;
                Debug.Log("Changed ArtyPrioritySystemEnabled to: " + state);
                return;
            }
            Debug.Log("Unable to parse ArtyPrioritySystemEnabled value: " + obj);
        }
