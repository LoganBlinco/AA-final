...
public class AdminCustomCommands : MonoBehaviour
{
	...
	public static Dictionary<string, Action<string[]>> customCommandDict = new Dictionary<string, Action<string[]>>()
	{
		...
		//comp
		{"s", command_compSlap},
		{"fun_cav",command_fun_cav },
		{"fun_nodrop",command_nodrop }
	};

	private static void command_flag(string[] obj)
	{
		ConsoleController.invoke("set triggerDynamicCapturePoint");
	}

	private static void command_nodrop(string[] obj)
	{
		ConsoleController.invoke("set characterWeaponApplyHorizontalDeviation false");
		ConsoleController.invoke("set characterWeaponFirearmApplyDrop false");
	}

	private static void command_fun_cav(string[] obj)
	{
		ConsoleController.invoke("vehiclesSpawn 100");
		ConsoleController.invoke("set vehicleHorseForAll true");
		ConsoleController.invoke("set allowFiring false");
		ConsoleController.invoke("vehiclesManager forceCappedSpeed false");
		string broadcast = "THIS IS A FUN ROUND. All players can ride on horseback. Firing has been disabled. Gallop to glory.";
		ConsoleController.broadcast_prefix(broadcast,AutoAdmin.f1MenuInputField);
	}
	...
	//runs server side input. Is useful for debugging 
	//rc [command]
	//eg: "rc ssinput set allowFiring false 
	//would have the SERVER input the command.
	...
	private static void command_acflag(string[] inputArray)
	{
		if (inputArray.Length == 1) //just "rc acflag"
		{
			//just call ac now.
			AutoAdmin.InputAllChargeCommands((int)AutoAdmin.currentTime);//calls straint away.
			ConsoleController.invoke("set triggerDynamicCapturePoint");
			AutoAdmin.broadcast_prefix("Capturable Flag has also been spawned. Quick go capture it!");
		...
		if (inputArray.Length == 2)
		{
			...
			if (int.TryParse(inputArray[1], out time) && time >= 0)
			{
				...
				AutoAdmin.CallAllChargeFlag((int)AutoAdmin.currentTime, acTime);
			}
		}
	}
