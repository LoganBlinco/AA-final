...
namespace eLF_AutoAdmin
{
	...
	public class ArtilleryChecker
	{
		...
		//need to update loads of code at some point.
		public static Dictionary<int, int> objectIDToPlayer = new Dictionary<int, int>();


		public static void OnInteractableObjectInteraction(int playerId, int interactableObjectID, GameObject interactableObject, InteractionActivationType interactionActivationType, int nextActivationStateTransitionIndex)
		{
			if (AutoAdmin.ArtyPrioritySystemEnabled)
			{
				HandleArtyPriority(playerId, interactableObjectID, interactableObject, interactionActivationType, nextActivationStateTransitionIndex);
			}
		...
		private static void HandleArtyPriority(int playerId, int interactableObjectID, GameObject interactableObject, InteractionActivationType interactionActivationType, int nextActivationStateTransitionIndex)
		{
			//check is interactable object already with a vlayuae?


			if (objectIDToPlayer.ContainsKey(interactableObjectID)) { return; }//a player has already claimed it so we can ignore.

			if (!SpacingDetection.startsWithCheck(interactableObject.ToString())) { return; } //object must be an artillery piece



			playerStruct playerInfo;
			//no player has claimed it yet. We need to check if the player interacting is a cannoneer to claim it.
			if (AutoAdmin.playerIdDictionary.TryGetValue(playerId, out playerInfo))
			{
				if (playerInfo._playerClass == PlayerClass.Cannoneer)
				{
					//will add the player to the dictionary and notify them
					objectIDToPlayer[interactableObjectID] = playerId;
					ConsoleController.privateMessage(playerId, MessagePresets.CannonClaimedMessage, AutoAdmin.f1MenuInputField);
				}
				//not cannoneer so we ignore.
			}
		}
