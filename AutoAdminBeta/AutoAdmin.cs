...
namespace eLF_AutoAdmin
{
    public class AutoAdmin : MonoBehaviour
    {
        ...
        public static bool ArtyPrioritySystemEnabled = false;
        ...
        public static Dictionary<PlayerClass, Dictionary<int, layerValues>> newClassInfomation = new Dictionary<PlayerClass, Dictionary<int, layerValues>>()
    {
        ...
        public static void CallAllChargeFlag(int timeForWarning, int flagTime)
        {
            //calls a flag at flagTime.
            //all charge is then called 30 seconds after.


            //wait delay seconds then, then warning then flag
            float minutes = Mathf.Floor(flagTime / 60); //could also use int division 
            float seconds = flagTime - 60 * minutes; //could also use mod.
            ...
            string flagMessage = "Flag capture point dropping at: ";
            string flagallchargeMessage = " , all charge will be called 30 seconds after flag is placed.";
            string command = string.Format("delayed {0} broadcast {4} {5} {1}:{2}{3} {6}", timeForWarning, minutes, tenSeconds, digits, MESSAGE_PREFIX, flagMessage, flagallchargeMessage);
            ConsoleController.invoke(command);
            ConsoleController.invoke(string.Format("delayed {0} set triggerDynamicCapturePoint", flagTime));
            string flagPlaced = "Flag has been placed! Capture it! All charge is in 30 seconds";
            ConsoleController.invoke(string.Format("delayed {0} broadcast {1}", flagTime, flagPlaced));

            int acTime = flagTime - 30;
            InputAllChargeCommands(acTime);
        }
        ...
        public static void InputAllChargeCommands(int acTime)
        {
            string command = string.Format("delayed {0} broadcast {1} {2}", acTime, MESSAGE_PREFIX, allChargeMessage);
            ConsoleController.invoke(command);
            ConsoleController.invoke(string.Format("delayed {0} set allowFiring false", acTime));
            ConsoleController.invoke(string.Format("delayed {0} set cannonMoverSpeedMultiplier 0", acTime));
            ConsoleController.invoke(string.Format("delayed {0} set cannonMoverRotationMultiplier 0", acTime));
            ConsoleController.invoke(string.Format("delayed {0} vehiclesManager forceCappedSpeed true", acTime));
            ConsoleController.invoke(string.Format("delayed {0} vehiclesManager cappedSpeed 0", acTime));
        }
        ...
        public static void playerKilled(int killerPlayerId, int victimPlayerId, EntityHealthChangedReason reason, string additionalDetails)
        {
            ...
            if (killerPlayerId == victimPlayerId) { return; }
        ...
        public static void InitialiseVariables()
        {
            ...
            ArtilleryChecker.objectIDToPlayer = new Dictionary<int, int>();
        }
