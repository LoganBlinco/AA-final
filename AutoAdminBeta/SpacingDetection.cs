...
namespace eLF_AutoAdmin
{
    public class SpacingDetection
    {
        ...
        public static object[] newFOL_Detector(int layerToCheck, int playerId, float safe_zone, float max_warning_distance, float damege_mod, int numberOfPlayersNeeded, int maxWarningDamege)
        {
            ...
            {
                Vector3 distanceWithoutHeight = hitCollider.gameObject.transform.position - AutoAdmin.playerIdDictionary[playerId]._playerObject.transform.position;
                distanceWithoutHeight.y = 0;//removes height from calculation
                float distSQR = Vector3.SqrMagnitude(distanceWithoutHeight);
